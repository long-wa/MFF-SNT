...
class WarmUPScheduler(LRScheduler):
    def __init__(self, optimizer, warmup, normal, epochs=50, last_epoch=-1):
        warmup = warmup.lr_spaces  # <c>
        normal = normal.lr_spaces  # <c>
        self.lr_spaces = np.concatenate([warmup, normal])  # <c>
        self.start_lr = normal[0]  # 0.00499999999999
...
def build_lr_scheduler(optimizer, epochs=50, last_epoch=-1):
    if cfg.TRAIN.LR_WARMUP.WARMUP:  # True
